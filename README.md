...
## База данных
База данных состоит из 6 таблиц, между которыми есть связь.

***users*** - хранит информацию о пользователях: 
* ***user_id*** - id чата с пользователем, primary key, является внешним ключом для аналогичного поля в других таблицах 
* ***selected_number*** - тут хранится номер, который пользователь выбирает на клавиатуре из цифр

***tasks*** - хранит информацию о задачах
* ***task_id*** - id задачи, primary key, авто инкремент
* ***user_id*** - id чата с пользователем
* ***task*** - здесь написана сама задача
* ***state_id*** - номер прогресса, foreign key (соответствие названия состояния номеру прогресса - в таблице state_id
* ***deadline*** - дата дедлайна в формате ISO
* ***generated_num*** - номер задачи в списке, который пользователь получает при нажатии команды /tasks
* ***completed_when*** - сюда записывается время, когда задача была отмечена как done. Было сделано для аналитики, чтобы отслеживать просроченные дедлайны

***states*** - статичная таблица, которая хранит соответствие между номер состояния и ео названием
* ***state_id*** - id состояния, primary key, не позволяет программисту вносить в tasks.state_id значения, которых нет в этой таблице
* ***name*** - название состояния

***habits*** - хранит информацию о привычках
* ***habit_id*** - id привычки, primary key, авто инкремент
* ***user_id*** - id чата с пользователем
* ***habit*** - здесь написана сама привычка
* ***amount*** - сюда предполагалось записывать количество выполнений для аналитики, но позже мы от этого отказались (стоит удалить это поле)
* ***is_done*** - 0 - если не сделано, 1 - если сделано
** ***generated_num*** - номер привычки в списке, который пользователь получает при нажатии команды /habits

***goals*** - хранит информацию о целях
* ***goal_id*** - id цели, primary key, авто инкремент
* ***user_id*** - id чата с пользователем
* ***goal*** - здесь написана сама цель
* ***is_done*** - 0 - если не сделано, 1 - если сделано
** ***generated_num*** - номер цели в списке, который пользователь получает при нажатии команды /habits

***habit_progress*** - таблица для аналитики по привычкам
* ***progress_id*** - id прогресса пользователя user_id за день date
* ***date*** - день за который считается количество выполненных задач
* ***user_id*** - пользователь, для которого считаются выполненные задачи
* ***progress*** - сохраняет процент выполненых задач 
